...
セマンティックレイヤー集約のための読取・分析 Skill。対象 PDS を使う workbook に散らばった**重複 calculated field** を見つけ、PDS へ寄せる候補を提示する。読取のみ（副作用なし）。workbook の書き換え自体は行わない。

## スコープ

含む:
- 対象 PDS の下流 workbook 列挙（Metadata API GraphQL）
- 対象 PDS を**直接 proxy する接続**（embedded datasource）に限定した CalculatedField の formula 収集。lineage（downstreamWorkbooks / upstreamDatasources）は推移的で派生 PDS 経由の workbook も混ざるため、上流閉包の差分で直接上流を復元して判定する
- formula 正規化（**コメント除去 + 空白吸収**。caption 非依存）での横断重複判定 → hoist 候補
...
含まない:
- PDS への注入・publish
- workbook 側 calc の削除・参照差し替え
- formula の意味的等価判定（字句正規化のみ。`SUM([A])/SUM([B])` と等価な別表現は別物として扱う）
- **説明文の生成・コメントの意味判定**（本 Skill は抽出のみで推論しない）
...
`candidates.json`: `pds_name` / `downstream_workbooks`（推移的合計）/ `downstream_workbooks_direct`（直接接続のみ。候補収集の母集団）/ `transitive_skipped`（派生 PDS 経由で除外した WB 名）/ `candidate_count` / `hoistable_count` / `candidates[]`。
...
`wb_descriptions[]` は各 WB calc の **構造化 description フィールド**を distinct 抽出したもの。`formula` はコメント（`//`, `/* */`）を残した raw。どちらも**抽出だけ**で、「コメントが説明か」「複数 description のどれを採るか」は判定しない。これらは `datasource-describer` の入力で、説明文の生成・衝突解決はそちらが担う。件数（0 / 1 / 2 件以上）ごとの採否・衝突解決の分岐は describer のワークフローを正とする。
