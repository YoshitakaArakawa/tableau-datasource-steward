...
`stale` の `revised_text` は `descriptions[]`（grain なら `datasource.description`）にも載せ、augmenter がそれを注入する。`audits[]` / `skipped[]` と provenance フィールド（`source` = `extracted` | `inferred`、`conflict`、`variants`、`previous_text`）は **orchestrator がユーザー報告で「抽出 / 推論」「衝突」「stale の原文」を区別するための注記**。augmenter は自身が定義したキー（`field_caption` / `text` / `caption` / `formula` / `datatype` / `description` / `datasource.description` 等）だけを読み、これらの注記は無視するので、change-set にそのまま残してよい。
...
- 説明は列名・型・サンプル・formula・WB 側 desc から「草案」を作る。最終文言の承認は途中確認ではなくワークフロー末尾の orchestrator 報告で得る
...
- grain は列 desc と別レイヤー。datasource が「何の 1 行か」を書き、注入は augmenter に委ねる
- 既存 desc・grain の検証は「主張 → 現データ照合 → valid/stale/unverifiable」。stale は修正草案 + 原文（`previous_text`）を残し、判断はユーザーに委ねる
- 全列カバレッジは「処理済みゼロ漏れ」を保証。書けない列は `skipped[]` に理由付きで残し、同義反復 desc で埋めない
