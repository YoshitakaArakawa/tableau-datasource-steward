...
PDS フィールドの description スロットを直接書く REST / Metadata API は無いため、この XML 注入 + republish が唯一の汎用経路。**calc field の説明もこの経路でのみ付与できる**（calc には継承元の上流列が無い）。source 列の Catalog description（`descriptionInherited` として継承される別スロット）は `update_source_column_descs.py` の担当で、本ファイルの範囲外。
...
calc の登録確認は GraphQL Metadata API（`fields{ ... on CalculatedField{ name formula } }`）で行う。`get-datasource-metadata` も calc を formula / description 付きで列挙するが、カタログのインデックス反映後にしか見えないため即時検証には使わない。
