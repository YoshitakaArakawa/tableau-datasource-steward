...
- **カバレッジ**: publish 後、GraphQL で物理列のうち desc 未設定の数を数え `coverage`（`regular_columns` / `described` / `undescribed` / `undescribed_columns`）を result に出す。GraphQL はカタログのインデックス依存のため、未インデックス時は `_note`、Overwrite では直近に足した desc の反映が 1 テンポ遅れて coverage が実態より 1 件少なく出ることがある（`roundtrip_checks` が当該編集の survive の権威。coverage は再読で確定する）。coverage は情報提供で `verified` の合否には含めない（部分整備は正当なユースケース）。
...
- Overwrite で grain を消さない（spec 未指定なら既存 grain を引き継ぐ）
