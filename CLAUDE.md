...
| Skill | 系統 | 責務 |
|---|---|---|
| `datasource-inspector` | READ | PDS のスキーマ・メタデータ・既存 calc を棚卸しし、整備 gap を洗い出す（既存 desc 全文・grain も事実として出す） |
...
4. `workbook-rewirer` → 各下流 WB を `CreateNew` で rewired 版として publish（augmented PDS へ付け替え + calc 参照差し替え）し、前後 view の画像並置比較まで実行（**ここまで承認プロンプトなしで自走**）
5. **orchestrator 報告**（このワークフローの最後）でユーザー確認を提示：
   ...
   - rewirer の `view_compare` の結果と並置レポート（`compare/view-compare.html`）への参照。export 失敗した view は原文エラー併記で先頭に、前後画像の目視所見も添える
...
- `datasource-augmenter` / `workbook-rewirer` の publish は **既定 `CreateNew`**（別名で新規コンテンツを作り、元を壊さない）。
- `Overwrite` は破壊的（PDS なら下流 workbook を巻き込みうる。workbook なら利用者の見ている本番 view を直接置換する）。**明示要求 + 影響の提示 + ユーザー承認**がそろったときのみ。
- steward は「更新案（augmented PDS / rewired WB）を作る」まで。本番の置換はユーザーの意図的操作。

### 承認ゲート
- 破壊的操作（**Overwrite publish（PDS / workbook とも）、PDS・workbook の削除、promote**）の前は必ずユーザー確認。真のゲートはここ。
...
- 読取系の一部は Tableau MCP。MCP は Metadata API をほぼ非対応のため、lineage / 既存 calc は `scripts/metadata_api.py` の GraphQL で自前に叩く。
...
セッション作業物（inspector / describer / prospector の出力 JSON、change-set spec、augmenter の out-dir、DL した .tdsx）は `work/<yyyymmdd>_<tag>/` に集約する。git 追跡は [work/README.md](work/README.md) のみ（規約詳細もそこに置く）。リポ直下や `.claude/` 配下に作業物を撒かない。
