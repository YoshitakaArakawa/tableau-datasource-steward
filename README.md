...
Tableau の Published Datasource は、実質的に組織のセマンティックレイヤーとして機能する。指標（計算フィールド）の定義、Dimension / Measure の区別、各列の意味（説明・コメント・フォルダ・デフォルト集計などのメタデータ）を保持するためだ。
...
この一文は Tableau Next / Data Cloud の用語解説ページにあり、「Tableau Next の Semantic Model に対応する、従来 Tableau 側の最も近いもの = Published Datasource」という対比で書かれている。本リポジトリのスコープ確定の論拠そのものである。
...
**対象外**: Tableau Next / Data 360 の「Tableau Semantics」「Semantic Model」。これは Salesforce のブランド製品名であり、別系統の API・オブジェクトモデルを持つ。上記の公式引用も、その両者を区別する文脈で書かれている。

リポジトリ名にも `semantic` をあえて使っていない（ブランド衝突回避）。「セマンティックレイヤーとして扱う」という思想は語るが、Tableau Next の機能ではない。

## 機能の核

- 既存 Published Datasource の**メタデータを埋める／整える**: 列の説明、デフォルト集計、フォルダ構成、Dimension / Measure 整理など
...
Repo 名 = Agent 名 = 内部 Skill 名 を `tableau-datasource-steward` で統一する。
...
- Agent パッケージとして提供するが、実装の中核は Skill に固める
- Skill の作成・設計時は、着手前に必ず `/creating-skills` を参照する
- 本リポジトリは将来 Public 化されうる前提で作る: 絶対パス・個人情報・実データを本文／コミット／サンプルに書かない、例示はダミー値、LICENSE は MIT を既定で置く
...
| Skill | 系統 | 責務 |
|---|---|---|
| `datasource-inspector` | READ | PDS のスキーマ・メタデータ・既存 calc を棚卸しし、整備 gap を洗い出す（既存 desc 全文・grain も出す） |
| `datasource-describer` | ANALYZE | 列・calc・datasource（grain）の説明草案を作り、既存 desc・grain が現データに適切かも検証。出所付きで change-set 化 |
| `workbook-calc-prospector` | ANALYZE | 下流 workbook の重複 calc を検出し hoist 候補を出す |
| `datasource-augmenter` | WRITE | change-set を受けて download → XML 編集 → publish → round-trip 検証 |
